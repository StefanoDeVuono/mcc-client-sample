				...
				if ($affected_rows > 0)
					...
					if (strlen($multi_alt_phones) > 5)
						...
						while ($r < $map_count)
							...
							if ($DB>0) {echo "DEBUG: add_lead query - $stmt\n";}
							$rslt=mysqli_query($db, $stmt);
							$Zaffected_rows = mysqli_affected_rows($db);
						...
						if ($custom_fields_enabled > 0)
							...
							if ($tablecount_to_print > 0) 
								...
								if (strlen($CFinsert_SQL)>3)
									...
									$rslt=mysqli_query($db, $custom_table_update_SQL);
									$custom_insert_count = mysqli_affected_rows($db);
									if ($custom_insert_count > 0) 
										...
										$rslt=mysqli_query($db, $vl_table_entry_update_SQL);
										$vl_table_entry_update_count = mysqli_affected_rows($db);
					...
					if ($callback == 'Y')
						...
						if ($camp_count > 0)
							...
							if ($valid_callback > 0)
								...
								if ($DB>0) {echo "DEBUG: add_lead query - $stmt\n";}
								$rslt=mysqli_query($db, $stmt);
								$CBaffected_rows = mysqli_affected_rows($db);
						...
						while ($search_found > $n)
							...
							if ( (strlen($VL_update_SQL)>6) or ($delete_lead=='Y') )
								...
								if ($delete_lead=='Y')
									...
									$rslt=mysqli_query($db, $stmt);
									$VCBaffected_rows = mysqli_affected_rows($db);
								...
								if ($DB>0) {echo "DEBUG: update_lead query - $stmt\n";}
								$rslt=mysqli_query($db, $stmt);
								$VLaffected_rows = mysqli_affected_rows($db);
							...
							if ($custom_fields=='Y')
								...
								if ($tablecount_to_print > 0) 
									...
									if ($delete_lead=='Y')
										...
										if ($DB>0) {echo "DEBUG: update_lead query - $stmt\n";}
										$rslt=mysqli_query($db, $stmt);
										$VCLDaffected_rows = mysqli_affected_rows($db);
										...
										if (strlen($update_SQL)>3)
											...
											$rslt=mysqli_query($db, $custom_table_update_SQL);
											$custom_update_count = mysqli_affected_rows($db);
										...
										if ($custom_update_count > 0)
											...
											$rslt=mysqli_query($db, $list_table_update_SQL);
											$list_update_count = mysqli_affected_rows($db);
...
function dialable_gmt($DB,$db,$local_call_time,$gmt_offset,$state)
	{
	$dialable=0;
...
##### Logging #####
function api_log($db,$api_logging,$api_script,$user,$agent_user,$function,$value,$result,$result_reason,$source,$data)
	{
	if ($api_logging > 0)
		{
		global $startMS;
