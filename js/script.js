...
(function() {
  ...
  setDataE = function(data) {
    ...
    for (_i = 0, _len = _ref.length; _i < _len; _i++) {
      ...
      if ($('#activeResourcesTable th.col8').is(':hidden')) {
        row += '<td class="col col8" style="display: none">' + element['calls'] + '</td>';
      } else {
        row += '<td class="col col8">' + element['calls'] + '</td>';
      }
      if ($('#activeResourcesTable th.col9').is(':hidden')) {
        row += '<td class="col col9" style="display: none">' + element['calls'] + '</td>';
      } else {
        row += '<td class="col col9">' + element['calls'] + '</td>';
      }
      if ($('#activeResourcesTable th.col10').is(':hidden')) {
        row += '<td class="col col10" style="display: none">' + element['calls'] + '</td>';
      } else {
        row += '<td class="col col10">' + element['calls'] + '</td>';
      }
      ...
      if ($('#activeResourcesTable th.col12').is(':hidden')) {
        row += '<td class="col col12" style="display: none">' + element['calls'] + '</td>';
      } else {
        row += '<td class="col col12">' + element['calls'] + '</td>';
      }
  ...
  setDataF = function(data) {
    ...
    $('#dialableLeeds h3').text(data['DIALABLE_LEADS']);
  ...
  $('#sectionD .options').on('click', 'input:checked', function(e) {
    var checkClass, colClass;

    checkClass = $(this).attr('class');
    colClass = $('#callsWaitingTable #' + checkClass).attr('class').split(' ')[0];
    $('#callsWaitingTable .' + colClass).show();
    return resizeColumns('#callsWaitingTable', 666);
  });

  $('#sectionD .options').on('click', 'input:not(:checked)', function(e) {
    var checkClass;

    checkClass = $(this).attr('class');
    return $('#callsWaitingTable th#' + checkClass + ' a').click();
  });
  ...
  $('#sectionE .options').on('click', 'input:not(:checked)', function() {
    ...
    checkClass = $(this).attr('class');
    return $('#activeResourcesTable th#' + checkClass + ' a').click();
  });
