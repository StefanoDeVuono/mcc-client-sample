...
(function() {
  var ajaxFunc, checkBoxes, checkListeners, dOrder, dSorter, droppedTrend, eOrder, eSorter, findF, findFajax, oldDropped, pause, resizeColumns, setDataA, setDataC, setDataD, setDataE, setDataF, visualizeC;
  ...
  setDataA = function(data) {
    $('#agent_ready .number').text(data['A']['agents_waiting']);
    $('#agent_total .number').text(data['A']['agents_logged_in']);
    $('#agent_incall .number').text(data['A']['agents_in_calls']);
    $('#out_total .number').text(data['A']['current_active_calls']);
    $('#agent_dead .number').text(data['A']['agents_in_dead_calls']);
    $('#agent_paused .number').text(data['A']['paused_agents']);
    $('#out_ring .number').text(data['A']['calls_ringing']);
    $('#out_live .number').text(data['A']['calls_waiting_for_agents']);
    $('#agent_dispo .number').text(data['A']['agents_in_dispo']);
    return false;
  };
  ...
  setDataE = function(data) {
    ...
    for (_i = 0, _len = _ref.length; _i < _len; _i++) {
      ...
      row += '<td class="col col15" data-session-id="' + element['session-id'] + '" data-station="' + element['extension'] + '" data-server-ip="' + element['server_ip'] + '"><a class="listen" href="#"></a><a class="speak" href="#"></a><a class="shout" href="#"></a></td>';
      row += '</tr>';
    }
  ...
  ajaxFunc = function() {
    return $.ajax({
      ...
      success: function(data) {
        ...
        return $("#activeResourcesTable").trigger("update");
      },
      dataType: "json"
    });
  };
  ...
  window.findAE = function() {
    window.Options.display.delay = window.Options.display.delay || 5;
    if (window.Options.display.delay > 0) {
      setTimeout(ajaxFunc, window.Options.display.delay * 1000);
      return false;
    } else if (window.Options.display.delay === 0) {
      $.ajax({
        url: "./find.php",
        data: {
          dSort: dSorter,
  ...
  $(document).ready(function() {
    var storeA, storeAjaxFunc, storeC, storeD, storeE, storeF;
    storeA = setDataA;
    storeC = setDataC;
    storeD = setDataD;
    storeE = setDataE;
    storeF = setDataF;
    return storeAjaxFunc = ajaxFunc;
  });
  ...
  $('#settingsPopup').on('click', 'input[type="submit"]', function(e) {
    ...
    window.Options.display.delay = parseInt($('#refreshrate').val());
...
\
  ...
  $('#activeResourcesTable').on('click', '.listen, .speak, .shout', function(e) {
    ...
    server_ip = $(this).parent().data('server-ip');
